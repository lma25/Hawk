...
/***/ function(module, exports, __webpack_require__) {
	...
	rtsp.RTSP_CONFIG['websocket.url'] = "ws://192.41.245.236:8000";//"ws://srv.tom.ru/ws/";
	rtsp.RTSP_CONFIG['log'] = _bp_logger.LogLevel.Error;

	document.addEventListener("DOMContentLoaded", function () {
	    var player = null;
	    setTimeout(function () {
	        player = rtsp.attach(document.getElementById('test_video'));
	        if (!player.started()) {
	            player.start();
	        }
	    }, 200);

	    var btn = document.getElementById('set_stream');
	    var url = document.getElementById('stream_url');
	    var video = document.getElementById('test_video');

	    btn.addEventListener('click', function () {
	        player.setStream(url.value);
	    });
	});
...
/***/ function(module, exports) {
	...
	var Logger = exports.Logger = function () {
	    function Logger() {
	        var level = arguments.length <= 0 || arguments[0] === undefined ? LogLevel.Debug : arguments[0];
	...
	Logger.level_map = (_Logger$level_map = {}, _defineProperty(_Logger$level_map, LogLevel.Debug, 'log'), _defineProperty(_Logger$level_map, LogLevel.Log, 'log'), _defineProperty(_Logger$level_map, LogLevel.Warn, 'warn'), _defineProperty(_Logger$level_map, LogLevel.Error, 'error'), _Logger$level_map);
	;
	...
	var RTSPPlayer = exports.RTSPPlayer = function () {
	    function RTSPPlayer(player, url) {
	        ...
	        if (this.isReplaced) {
	            ...
	            this.connection = new _connection.RTSPConnection(parsed.host, parsed.port, parsed.urlpath, {}, _connection.RTSPWebsocketBackend);
	                ...
	                if (lastDTS !== undefined) {
	                    ...
	                    //mp4Sample.duration = sampleDuration;
	                        ...
	                        if (nextAvcDts) {
	                            ...
	                            // if fragment are contiguous, or delta less than 600ms, ensure there is no overlap/hole between fragments
	                            /*/ RTSPStream.SCALE_FACTOR*/if ( /*contiguous ||*/Math.abs(delta) < 600) {
	                                if (delta) {
	                                    if (delta > 1) {
